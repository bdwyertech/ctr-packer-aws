...
// Ensure ReleasesGetter implements the Getter interface
var _ plugingetter.Getter = new(ReleasesGetter)
...
func (g *ReleasesGetter) Get(what string, opts plugingetter.GetOptions) (io.ReadCloser, error) {
	...
	case "sha256":
		...
		url := filepath.ToSlash(g.BaseURL + ghURI.PluginType() + "/" + opts.VersionString() + "/" + ghURI.PluginType() + "_" + opts.VersionString() + "_SHA256SUMS")
	...
	case "meta":
		...
		url := filepath.ToSlash(g.BaseURL + ghURI.PluginType() + "/" + opts.VersionString() + "/" + ghURI.PluginType() + "_" + opts.VersionString() + "_manifest.json")
		req, err = http.NewRequest("GET", url, nil)
	case "zip":
		// https://releases.hashicorp.com/packer-plugin-docker/1.1.1/packer-plugin-docker_1.1.1_darwin_arm64.zip
		url := filepath.ToSlash(g.BaseURL + ghURI.PluginType() + "/" + opts.VersionString() + "/" + opts.ExpectedZipFilename())
		req, err = http.NewRequest("GET", url, nil)
	default:
...
func (g *ReleasesGetter) Init(req *plugingetter.Requirement, entry *plugingetter.ChecksumFileEntry) error {
...
func (g *ReleasesGetter) Validate(opt plugingetter.GetOptions, expectedVersion string, installOpts plugingetter.BinaryInstallationOptions, entry *plugingetter.ChecksumFileEntry) error {
...
func (g *ReleasesGetter) ExpectedFileName(pr *plugingetter.Requirement, version string, entry *plugingetter.ChecksumFileEntry, zipFileName string) string {
